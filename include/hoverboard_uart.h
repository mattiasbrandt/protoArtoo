...
// batteryRaw  = V x 100; divide by 100.0 for volts.
// boardTempRaw = degC x 10; divide by 10.0 for degC.
// currentL/currentR = A x 100 (Gen2.x only; 0 for FOC firmware).
// -----------------------------------------------------------------------------
struct HoverboardFeedback {
    int16_t batteryRaw;    // V x 100 (divide by 100.0 for volts)
    int16_t boardTempRaw;  // degC x 10 (divide by 10.0 for degC)
    int16_t speedR;        // right motor speed, RPM
    int16_t speedL;        // left motor speed, RPM
    int16_t currentL;      // left motor current x 100 = A (Gen2.x only; 0 for FOC)
    int16_t currentR;      // right motor current x 100 = A (Gen2.x only; 0 for FOC)
};
