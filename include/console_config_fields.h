...
//     (read/write through the toggle Setting's declaration).
...
//
// `paramKey` is the Component Toggle Setting's form name, as its declaration in
// src/config_settings.cpp spells it: the Console reads and writes the toggle
// through that declaration (ADR 0068, amended 2026-09-26), and a name that
// matches no declaration reads as off and is refused on write. The member
// pointer is only the bit order of the Active Component Toggle snapshot. There is
...
struct ComponentToggleField {
    ...
    const char* paramKey;       // the toggle Setting's form name (src/config_settings.cpp)
