...
//
// The five fixed servo field sets, reduced to their NVS keys (#286, #345,
// ADR 0041).
//
...
// on which path read it. #423 deleted the API names: an Output is read and
// written as its row (ADR 0068), and a backup made while /api/config still
// spoke `arm1OpenUs` is translated once, at the backup seam
// (data/maintenance.js), and nowhere here.
//
// What could not be deleted is the keys. A controller that has not saved a
// row yet still holds its calibration under the NVS keys below. They are read
// once, by the row loader, on a row nothing has written -- and never written
// again. Deleting that read would not delete a builder's calibration; it would
// silently drop it, which is the one thing #286 refuses.
// configSaveServoOutputs() removes the keys once the rows they became are
// safely down.
//
// This table is the only statement anywhere of which Output Address each set
// was ever about: the keys carry their channel in their spelling and nowhere
// else, which is exactly why an expander could never be another one of them.
// The table stores no value. It is deleted whole when that reason has gone,
// and ServoLegacyNarrowing below goes with it.
//
...
inline constexpr ServoLegacyFieldSet SERVO_LEGACY_FIELD_SETS[SERVO_LEGACY_FIELD_SET_COUNT] = {
    {LEDC_CH_ARM1, "arm1_op", "arm1_cl", "arm1_type"},
    {LEDC_CH_ARM2, "arm2_op", "arm2_cl", "arm2_type"},
    {LEDC_CH_AUX1, "aux1_op", "aux1_cl", "aux1_type"},
    {LEDC_CH_AUX2, "aux2_op", "aux2_cl", "aux2_type"},
    {LEDC_CH_AUX3, "aux3_op", "aux3_cl", "aux3_type"},
};
