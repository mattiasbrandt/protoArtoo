...
//   16 channels x 11 bits packed sequentially, LSB first, starting at byte 1
...
// -----------------------------------------------------------------------------
// sbusUnpackChannels()
// Unpack 16 x 11-bit SBUS channels from a 22-byte packed payload.
//
// Parameters:
...
inline void sbusUnpackChannels(const uint8_t* data, int16_t* ch) {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        // Read 3 bytes (safe: 16 ch x 11 bits = 176 bits = 22 bytes,
