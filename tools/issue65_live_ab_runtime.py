...
def execute_run(report: Mapping[str, Any]) -> int:
    ...
    try:
        ...
        # Load deployment identity from planner's generated version files instead of broken deploy_pair
        plan = report["plannerPlan"]
        worktree = Path(plan["paths"]["worktree"])
        fw_version_file = worktree / "data" / "fw-version.json"
        fs_version_file = worktree / "data" / "fs-version.json"
        
        try:
            with open(fw_version_file, "r", encoding="utf-8") as f:
                fw_data = json.load(f)
            with open(fs_version_file, "r", encoding="utf-8") as f:
                fs_data = json.load(f)
        except (OSError, json.JSONDecodeError) as error:
            raise Issue65RuntimeError(
                f"could not load generated version files from {worktree}/data/: {error}"
            )
        
        # Build deployment dict with the same structure as the old deploy_pair returned
        deployment = {
            "firmware": {
                "generatedIdentity": fw_data,
            },
            "filesystem": {
                "generatedIdentity": fs_data,
            },
        }
        arbiter = StopArbiter(
