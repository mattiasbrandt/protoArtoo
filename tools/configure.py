...
DEFAULTS = {
    ...
    # No UPLOAD_PORT default on purpose. "/dev/ttyUSB0" used to sit here and be
    # written into user.mk unconditionally, which made a guess look like the
    # operator's own choice -- and a user.mk value is honoured without even the
    # ambient-environment warning tools/resolve_upload_port.py prints. Left
    # blank, the port is resolved (or refused) at flash time against the boards
    # actually attached. See the Makefile's "USB upload port" section.
}
...
def _render_user_mk(ota_ip: str, build_env: str, upload_port: str) -> str:
    """Render user.mk. An empty `upload_port` omits the line entirely, so the
    Makefile resolves the port per flash instead of pinning one board."""
    content = (
        "# user.mk — generated by tools/configure.py; do not commit\n"
        f"OTA_IP      = {ota_ip}\n"
        f"BUILD_ENV   = {build_env}\n"
    )
    if upload_port.strip():
        content += f"UPLOAD_PORT = {upload_port.strip()}\n"
    return content
...
def run() -> int:
    ...
    upload_port = _text(
        "USB upload port (blank = detect at flash time, and refuse if ambiguous):",
        default=existing.get("UPLOAD_PORT", ""),
    )
