...
|---|---|---|---|---|---|---|---|---|
| **DM:VADER** | `$M` Imperial March | none | `DV:VADER` (red MARCH) | `DV:VADER` (red MARCH) | `DV:VADER` (red 47s) | effect resets only (no panels) | raw `@0T11/@0P11/@HPA0021\|47` replaced by `DV:VADER` (task #5), dome-source-confirmed shares ROCKMARCH preset shape | `DV:VADER` swap **software-verified**; awaiting on-droid confirm after ROCKMARCH gate |
...
| **DM:LEIA** | `$L` | none | `DV:LEIA` | `DV:LEIA` | `DV:LEIA` (Leia holo 36s) | effect resets only | raw `@0T6/@0P6/@HPS101/HPR02/HPT02\|36` replaced by `DV:LEIA` (task #5); body-inferred pending codex `DomeSequences.h` confirm | `DV:LEIA` swap **software-verified**; not visually verified |
| **DM:ALARM** | category | none | `DV:ALARM` | `DV:ALARM` | `DV:ALARM` (red 10s) | effect resets only | raw `@0T3/@0P3/@HPA0021\|10` replaced by `DV:ALARM` (task #5); body-inferred pending codex confirm | `DV:ALARM` swap **software-verified**; not visually verified |
| **DM:HEART** | category | none | `@1MYou're Wonderful` (raw text, body-owned) | `DV:HEART` | `DV:HEART` rainbow | effect resets only | raw `@1P2/@HPF/HPR/HPT006\|10` replaced by `DV:HEART` (PSI/holo); front text stays raw, sent AFTER the preset; body-inferred pending codex confirm | `DV:HEART` swap **software-verified**; not visually verified |
...
| **DM:CANTINA** | `$C` | LOOP: ring+pie open/close mix | `DV:CANTINA` | `DV:CANTINA` | `DV:CANTINA` (white 15s) | in-steps closes | raw `@0T2/@0P2/@HPA0029\|15` replaced by `DV:CANTINA` (task #5; loop header now step index 2); opens pies; dense loop cadence (queue risk); body-inferred pending codex confirm | `DV:CANTINA` swap **software-verified**; do-not-test-yet (pie movement) |
...
| **DM:SCREAM** | category | opens ALL pies+ring, LOOP random flutter/open | `DV:SCREAM` | `DV:SCREAM` | `DV:SCREAM` | in-steps / mask | raw `@0T5/@0P5/@HPA0070/@HPA105\|5` replaced by `DV:SCREAM` (task #5; flutter loop header now step index 15); opens pies; dense + random; body-inferred pending codex confirm | `DV:SCREAM` swap **software-verified**; do-not-test-yet (pie movement) |
| **DM:OVERLOAD** | category | random ring+pie **flutter** (`:OF`) | `DV:OVERLOAD` | `DV:OVERLOAD` | `DV:OVERLOAD` | flutters don't mark open → **no terminal close** (native-test verified) | raw `@1T4/@2T4/@0P4/@HPA0070` replaced by `DV:OVERLOAD` (task #5); flutters pies (movement); body-inferred pending codex confirm | `DV:OVERLOAD` swap **software-verified**; do-not-test-yet on hw (pie movement) |
