...
|---|---|---|---|---|---|---|---|---|
| **DM:VADER** | `$M` Imperial March | none | `DV:VADER` (red MARCH) | `DV:VADER` (red MARCH) | `DV:VADER` (red 47s) | effect resets only (no panels) | raw `@0T11/@0P11/@HPA0021\|47` replaced by `DV:VADER` (task #5); **dome-source-confirmed identical to `DV:ROCKMARCH`** (both `domeApplyMarchVisuals()`, codex 2026-06-21) | `DV:VADER` swap **software-verified**; awaiting on-droid confirm after ROCKMARCH gate |
...
| **DM:LEIA** | `$L` | none | `DV:LEIA` (LEIA) | `DV:LEIA` (LEIA) | `DV:LEIA` (HPS101 front, rear/top off, 36s) | effect resets only | raw `@0T6/@0P6/@HPS101/HPR02/HPT02\|36` replaced by `DV:LEIA` (task #5); **dome-source-confirmed** (codex 2026-06-21) | `DV:LEIA` swap **software-verified**; not visually verified |
| **DM:ALARM** | category | none | `DV:ALARM` (ALARM) | `DV:ALARM` (ALARM) | `DV:ALARM` (red 10s) | effect resets only | raw `@0T3/@0P3/@HPA0021\|10` replaced by `DV:ALARM` (task #5); **dome-source-confirmed** (codex 2026-06-21) | `DV:ALARM` swap **software-verified**; not visually verified |
| **DM:HEART** | category | none | `DV:HEART` (FLD scroll text "You're\nWonderful"; RLD untouched) | `DV:HEART` (front FLASHCOLOR; rear untouched) | `DV:HEART` rainbow (HPF/HPR/HPT006\|10) | effect resets only | raw `@1P2/@HPF/HPR/HPT006\|10` **and** the body's old raw `@1MYou're Wonderful` replaced by `DV:HEART` — the dome owns the (two-line) text natively; body has no structured text step. **dome-source-confirmed** (codex 2026-06-21) | `DV:HEART` swap **software-verified**; not visually verified |
...
| **DM:CANTINA** | `$C` | LOOP: ring+pie open/close mix | `DV:CANTINA` | `DV:CANTINA` | `DV:CANTINA` (white 15s) | in-steps closes | raw `@0T2/@0P2/@HPA0029\|15` replaced by `DV:CANTINA` (task #5; loop header now step index 2); logics **blue** (FLASHCOLOR `kBlue`), holos white; opens pies; dense loop cadence (queue risk); **dome-source-confirmed** (codex 2026-06-21) | `DV:CANTINA` swap **software-verified**; do-not-test-yet (pie movement) |
...
| **DM:SCREAM** | category | opens ALL pies+ring, LOOP random flutter/open | `DV:SCREAM` | `DV:SCREAM` | `DV:SCREAM` | in-steps / mask | raw `@0T5/@0P5/@HPA0070/@HPA105\|5` replaced by `DV:SCREAM` (task #5; flutter loop header now step index 15); REDALERT logics/PSI + short-circuit/wag holos; opens pies; dense + random; **dome-source-confirmed** (codex 2026-06-21) | `DV:SCREAM` swap **software-verified**; do-not-test-yet (pie movement) |
| **DM:OVERLOAD** | category | random ring+pie **flutter** (`:OF`) | `DV:OVERLOAD` (FAILURE) | `DV:OVERLOAD` (FAILURE, 12s) | `DV:OVERLOAD` (HPA0070) | flutters don't mark open → **no terminal close** (native-test verified) | raw `@1T4/@2T4/@0P4/@HPA0070` replaced by `DV:OVERLOAD` (task #5); flutters pies (movement); **dome-source-confirmed** (codex 2026-06-21) | `DV:OVERLOAD` swap **software-verified**; do-not-test-yet on hw (pie movement) |
...
- **DM:ROCKMARCH** — hardware-verified for panels: no brownout, music stops at end, no group
  ...
  P1/all panels now seated. FLD/PSI/holo visual parity addressed by task #5: the raw
  `@0T11/@0P11/@HPA0021|47` lines were replaced by `DV:ROCKMARCH` (dome-source-confirmed),
  and the same `DV:<name>` swap is now applied across VADER/CANTINA/LEIA/ALARM/HEART/SCREAM/
  OVERLOAD (software-verified; on-droid visual confirm gated behind the ROCKMARCH hardware test).
