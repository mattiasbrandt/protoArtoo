...
open/closed is drawn in color, the legend is a fixed one-word list, the "Other
...
**Color stays reserved.** What a part *is* is shown by shape. #327's two meanings
— amber for *you can act on this*, red for *stopped or refused* — remain the whole
of color on this surface.
...
- **Color by Part Kind**, on the grounds that what a part is, is not a state.
  Rejected: **Status Color** says color carries *exactly two* meanings, and a
...
- **#298's route obligation is honoured without an exemption.** The selection
  ...
  panel; this one does.
- **`CONTEXT.md` **Part Kind** said it *"colors a surface"*, which #327's palette
...
- **Open and closed are shown by color, not geometry.** An open Part's footprint fills with the interaction accent and its stroke brightens; closed spends no color; the footprint never leaves its own bay. This is the pattern the interactive dome already uses (`data/dome_panel_model.js`). Operator: *"open close should simply be represented with the colors (same as we did with the interactive dome design)"*. It replaces "open is drawn open" and narrows *Color stays reserved*: amber still marks ends not measured, red still marks stopped or refused, and the accent now also marks an open Part on a droid picture.
