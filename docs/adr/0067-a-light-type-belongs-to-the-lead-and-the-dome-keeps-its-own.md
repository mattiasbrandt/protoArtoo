# A light type belongs to the lead, and the dome keeps its own
...
of this lead*. Nothing said so, `rgb` sat inside the servo list, and it was chosen
...
model. A **Part** inherits the type of the lead it is on; it does not carry one.
...
1. **A Light Type on the Part.** Reads naturally - *"the Data Panel is lit by an
   ...
   claiming hardware that is really a fact about the lead. The Part inherits it,
...
4. **Wiring loses the LED strip question; a light's lead comes from the Part's
   Output assignment**, as a servo's does. Tidier, one answer instead of two.
   Rejected by the operator: what is physically on a lead is a wiring fact, and
   Wiring's charter is *"I am holding a lead: where on this board does it go?"*.
5. **Build the per-lead firmware change and the surface together.** No builder
...
- **The Lights surface carries no Output name, pin or board label.** Where a
  lead goes is Wiring's answer, and it is reached from here rather than repeated.
- **ADR 0045's deferral to #319 is spent.** #319 closed without settling body
  light outputs, so the per-lead change now hangs off this decision instead.
