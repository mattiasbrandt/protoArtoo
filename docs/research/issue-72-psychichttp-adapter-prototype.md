...
**Third revision (this one) — same class of mistake a third time, in the same function.** While dogfooding #73's `--build-env` harness fix, `tools/ota_upload.py` hung indefinitely at "Sending invitation" against the (by-then-working) prototype. `startHttpServerOnce()`'s early return for `PA_USE_PSYCHICHTTP_PROTOTYPE` sat before mDNS init *and* ArduinoOTA task startup, not just the ESPAsyncWebServer route registration it was meant to replace — all three live in one function, and the return skipped everything after it, exactly the "skip too much of a multi-purpose function" mistake already made once for WiFi bring-up in the second revision. Fixed (commit `34df122`) by branching only the HTTP-server-specific block; mDNS and ArduinoOTA now run unconditionally for both backends. Verified live: `protoartoo.local` resolves again, and a full OTA round-trip succeeds.

**Standing lesson, now reinforced three times on this one function**: before gating any code path inside an existing multi-purpose function, read the *entire* function first and enumerate everything it does — not just the one responsibility being replaced. An early return is the easiest way to silently drop unrelated things bundled into the same function. This applies beyond this prototype; it's a general hazard whenever swapping one piece of a function that also happens to own several other unrelated setup steps.
