...
- drive integration remains hoverboard UART contract based
...
|---|---|---|---|---|
| protoArtoo | ESP32 body controller on Artoo PCB profile | RC receiver modes (PWM/SBUS) plus browser UI | Hoverboard UART drive focus plus body-owned audio modules (DY-SV5W primary) | fully open repository target (firmware + web + docs) with no binary-only paywall goal |
...
| Hoverboard UART-centric drive focus | ✅ explicit technical target | ❌ public baseline uses Sabertooth/SyRen drive stack | ❌ public baseline uses Sabertooth/SyRen drive stack | ❌ documented stack centers on Sabertooth/SyRen plus MarcDuino ecosystem | ❌ public baseline emphasizes ESC and Sabertooth/SyRen modes | ❓ unknown/integration-dependent | ❌ CAN module ecosystem focus, not hoverboard UART focus |
