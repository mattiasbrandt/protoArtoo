...
**Surface Anatomy**:
The shape every operator surface follows inside the **Operator Shell**, so a builder who has learned one screen has learned them all: a title that is the name in the nav and nothing under it, sections whose heading carries a count, a state or a provenance and never appears bare, the work area (a table, a picture or a form), the acts available named beside the thing they act on, and a feedback line. It is drawn under one identity, the inside of the droid - an instrument panel: dark surfaces, the dome's panel lines as structure, the droid's blue as the single accent, mono readouts, icons from one small drawn set and no emoji - so the theme comes from structure and colour rather than ornament. It carries what #327 fixed (dark only, amber and red reserved for state, computer resolution) and adds what #327 left to "the frontend work": where things sit. A surface that departs from it is a defect, not a taste (#395, ADR 0066, amended 2026-09-18).
