...
**Surface Anatomy**:
The shape every operator surface follows inside the **Operator Shell**, so a builder who has learned one screen has learned them all: a title with the question it answers beneath it, sections whose heading carries a count, a state, a provenance or a purpose and never appears bare, the work area (a table, a picture or a form), the acts available named beside the thing they act on, and a feedback line. It is drawn under one identity, the inside of the droid - an instrument panel: dark surfaces, the dome's panel lines as structure, the droid's blue as the single accent, mono readouts, icons from one small drawn set and no emoji - so the theme comes from structure and colour rather than ornament. It carries what #327 fixed (dark only, amber and red reserved for state, computer resolution) and adds what #327 left to "the frontend work": where things sit. A surface that departs from it is a defect, not a taste (#395, ADR 0066).
_Avoid_: design system (a component library this project does not ship), theme or skin (the identity is part of the anatomy, not a switchable layer), template, workspace, a generic web app look
...
**Lights**:
The destination that answers *what lights my droid has, how each one is lit, and what I can tell it to do* - every **Part** whose **Part Kind** is a light, grouped by where it sits. It is not a page about any one lighting product, and an LED strip is a **Light Type** rather than a thing on the droid (operator, 2026-09-20: *"LED strip is simply one of the differed light types we can assign to a droid part"*). A light protoArtoo drives names its **Light Type** and its settings, and reads *on*, *off* and *flash* with brightness for *how far* - the words ADR 0049 gives every Part, so one word means one thing across the droid. A **dome light** names no Light Type, because the dome controller owns its hardware; it offers that controller's own modes and colours instead, under the labels a **Sequence** already shows, and what it reports is commanded intent and never device truth (ADR 0045). Which lead carries a light is **Wiring**'s answer, not this one. A row added to the **Droid Parts Catalog** appears here with no code change, and a light nobody has fitted still appears - *choreograph first, wire later*.
...
**Part Kind**:
What a **Part** usually is — servo-driven, a light or display, an indicator. Successor to the catalog's `lit:` annotation, and **advisory in principle, never a constraint**: it gives a Part its own treatment on a surface and lets one query a surprising mapping, and it never refuses one, because plenty of builds move something the reference drawing shows as a display (#320). A light Part does not carry how it is lit - that is its lead's **Light Type**, and a light on no lead of ours has none.
_Avoid_: part type, part class, component type (that one names what is fitted to an **Output**, not what the Part is)

**Light Type**:
What protoArtoo puts on the end of one of its own leads to light a **Part** - *LED strip* today, and a list that grows. It is the light half of the same answer a servo gives as its model, because both say what is physically on that lead; it is therefore a property of the **Output**, and a **Part** inherits the type of the lead it is on. **A dome light has no Light Type**: the dome controller owns its hardware, and protoArtoo reaches it by forwarding documented commands (ADR 0045). A droid may have **several lit body Parts, each on its own lead** with its own type and settings (operator, 2026-09-20).
_Avoid_: light kind (that is **Part Kind**), LED type, a lighting product as a type (*AstroPixels* and *Teeces* name a product a builder fits, not what is on our lead), "AUX LED", a Light Type on a **dome light**

**Output**:
An addressed row that drives one or more **Part**s: an **Output Address**, the Parts it drives, and a **kind** saying what is on the end of the lead. Usually one - but a lead ganged to two parts that always move together names both, so nothing physically driven is invisible to the software; two Parts on one Output cannot move independently, which is a **Rehearsal Warning** rather than a refusal (#296). Rows are mixed by design — a servo, an RGB strip and an indicator chain sit in one table — because an Output Address is where a lead plugs in, whatever it plugs into. What is on the lead is one answer in two vocabularies: a servo's model where it drives a servo, a **Light Type** where it lights something. Kind decides which columns a row carries at all, so a light row is not a servo row with five meaningless fields (#320).
...
**Part**:
What the droid is made of, part by part — a utility arm, a charge bay door, a dome pie, a PSI, the Magic Panel — identified by its key in the **Droid Parts Catalog** and carrying a **Part Kind**. A light is a Part exactly as a panel is, and a device that both moves and lights is several Parts, one per thing an **Output** drives — a holoprojector is a pan Part, a tilt Part and a light Part (#320). Sequences and RC bindings reference the Part; an **Output** records which Part it drives. Part names are identity; the **Output Address** is where the lead plugs in. A Part being *known* and a Part being *driveable here* are separate facts: naming one no output records is legal to author and reports `part-not-assigned` when run, so the droid's own wiring - not the catalog - decides what moves (#301). A Part is driven by **at most one Output**: assigning it to another Output *moves* it rather than sharing it, which makes the conflict unrepresentable instead of merely reported. Operator surfaces label a Part with its plain-English alias - *Left body door*, *Dome pie 1* - with the Printed Droid shorthand alongside on dome parts, and the `cad_name` reaching the bench through **Wiring**, the declared bridge between the two naming systems. A builder does not rename a Part, so a screenshot, a support request and a shared sequence read the same on every droid (#296).
