...
**Component Protocol**:
The wire contract firmware speaks to a **Component Member**, and the level at which drivers are actually written. One protocol commonly serves several products, and it is **not owned by a Component Family**: Dimension Engineering packet serial drives a Sabertooth under Foot Drive and a SyRen under Dome Rotation from one implementation, differing only by address. This split is what makes a lineup cheap to extend — a product speaking a protocol we already have is a **Component Registry** entry, while one needing a new protocol is a driver's worth of work, and the registry is where that difference is visible before anyone commits.
_Avoid_: driver (that names the code, and implies one per family), backend, transport (that names the wire, not the contract spoken over it)
