...
**Restore Group**:
One of the three parts of a backup a builder may restore on its own: **Choreography** (the **Sequence**s they authored), **The droid** (its **Droid Build**, **Fitted Parts**, Part-to-**Output** assignments and calibration, component config and audio roles), and **Tonight** (the **RC Map**). Three because they go stale at three different rates, and because Tonight is the one with a clock on it - since #330 the RC Map is the running order, so restoring a month-old one before a show replaces what the droid will do. A restore **replaces** within a group and never merges, so the 16-sequence cap cannot be exceeded and a builder knows what they have without reading a report; a copy of what is about to go is offered first, and a failed save stops the replace. Restoring onto a **different board** writes only the groups that name no pin and says on the receipt what it left behind. Each group's *touches / leaves alone* lines are written against these three and no others (#294, ADR 0056).
_Avoid_: section (that is the shipped checkbox label), partial restore, merge, a group per endpoint
