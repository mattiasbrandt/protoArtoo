...
**Activity Group**:
How the nav is ordered — by the job a builder is doing rather than by firmware subsystem: **Drive**, **Perform**, **Configure**, **Maintain**. A surface may appear in more than one group, because Sound and Dome are reached for both when driving and when authoring; a group is a way to find something, never a claim to own it. The members: **Drive** carries Foot Drive, Dome, Sound and RC; **Perform** carries Sequences, Sound and Dome; **Configure** carries Configuration, **Parts** and **Wiring**; **Maintain** carries Maintenance, WiFi and Firmware (#288). **Dashboard** sits outside the groups as the landing, since it answers *what is my droid doing* rather than *what am I doing*, and guided **Setup** sits outside as a takeover rather than a destination (#325).
_Avoid_: one home per surface, workspace, a group that owns its members, grouping by subsystem

**Surface Anatomy**:
The shape every operator surface follows inside the **Operator Shell**, so a builder who has learned one screen has learned them all: a title that is the name in the nav and nothing under it, sections whose heading carries a count, a state or a provenance and never appears bare, the work area (a table, a picture or a form), the acts available named beside the thing they act on, and a feedback line. It is drawn under one identity, the inside of the droid - an instrument panel: dark surfaces, the dome's panel lines as structure, the droid's blue as the single accent, mono readouts, icons from one small drawn set and no emoji - so the theme comes from structure and color rather than ornament. It carries what #327 fixed (dark only, amber and red reserved for state, computer resolution) and adds what #327 left to "the frontend work": where things sit. A surface that departs from it is a defect, not a taste (#395, ADR 0066, amended 2026-09-18).
...
**Status Plate**:
The one recessed instrument plate the **Operator Shell** carries on every surface, holding the chips that say what the droid is doing. A chip earns its place only if seeing it would change what the operator does next - which is what cuts eight chips out of the 112 keys `/api/events` streams: estop, drive armed, RC link, **Non-RC Control**, **Sleep Mode**, speed preset, **protoR2link** and sound link, ordered by what bites fastest rather than by subsystem. It shows **values, not exceptions**, in fixed positions, so it is read by muscle memory rather than scanned; a plate that is empty when all is well cannot be told from a plate that has stopped updating. Telemetry - uptime, heap, signal strength, loop rate - is **Dashboard**'s and never the plate's. A chip is a **route, not a control**: clicking one goes to the surface where that thing is changed and never changes it, the **Latching Estop** being the single exception, which acts in place (#325). The whole plate carries **one freshness state**, because everything on it arrives on one stream and its age is one fact: the chips keep their last values and the plate says how old they are - *still finding out*, and therefore never amber (#324, #327).
_Avoid_: status bar, a pill per page, floating boxes, a plate that blanks when the stream drops, telemetry on the plate, a per-chip freshness marker

**Ignored Input Notice**:
The rate-limited notice the **Operator Shell** shows when input arrives for something that cannot act on it - a stick moved while the feet are disarmed, a command sent into a latched **Estop**, a motion request with no **Non-RC Control** consent. It names what is off, and that thing's **Status Plate** chip is the route to where it is changed, so the notice says *what* and the chip says *where*. It exists because the silence is otherwise total: the reference's own UX review made *"nothing on screen tells you the drive is disarmed when you move a stick"* its first finding, and the operator's eyes are on the droid, not the page. Rate-limited rather than per-event because RC input arrives at 50 Hz (#324).
...
**Backup**:
The file a builder saves to their computer holding what they made on the controller, and the three parts a restore may write on their own: their **Sequence**s, the droid's **Configuration**, and the **RC Map**. Three because they go stale at three different rates - a Sequence changes when the builder writes one, Configuration when a linkage is rebuilt, and the RC Map on the day, since it is the running order and restoring a month-old one before a show changes what the droid will do (#330). A restore **replaces** the part it writes and never merges into it, and a file holding more Sequences than the droid stores restores the first ones in file order up to its cap and names the rest on the receipt (ADR 0056, amended 2026-09-25), so a builder knows what they have without reading a report; a copy of what is about to go is offered first, and a failed save stops the replace. Restoring onto a different **Board Variant** writes only the parts that name no pin, and says on the receipt what it left behind. Today the file carries four of those parts and not the Sequences, while the page promises to save every setting (#294, ADR 0056). The Configuration is **written in the shape it is read**: a restore sends back what the backup holds, an **Output** as the addressed row the droid reports, and an older backup is translated once, at the restore, and nowhere else (ADR 0068).
_Avoid_: partial restore, merge, a part per endpoint, a name the restore panel does not already use, flattening what was read into what is written

**Wiring**:
The destination that answers *"I am holding a wire: where on this board does it go?"* - one diagram of the running **Body Controller**, every wire leaving it (its **Output**s and its **Board Lane**s), and the printable wiring sheet, which is the same document from one generator so the bench copy and the screen copy cannot disagree; the sheet leaves out the *Unused* list the screen ends on. *Which Part an Output moves* is **Parts**' question, not this one: the per-Part lists left Wiring on the operator's review (2026-09-19, #411). The Outputs it draws are whatever the running firmware reports (operator: *"the outputs is supposed to be dynamic, thats the whole point of the wiring and "mapping" we have"*). It is a reference first: the sheet and its generator write nothing. The one thing a builder sets here is which **Output**s are wired, and which **Output** carries the LED strip - because that is a fact about what is plugged in where (operator, 2026-09-18, #369); what each Output's servo is stays on Servos. It is rendered from the **Board Lane**s the running firmware reports, and **each wire is named first by what is printed on the board** - the PCB silkscreen, as `docs/pin_map.md` records it (on artoo-esp32 every one of them is printed ARM1-ARM5) - and then by what protoArtoo calls it. The board is shown as the builder's chosen **Body Controller** product picture, not as a word. Each wire takes **its own color and line type**, the way a real loom is told apart; that color names a wire and carries no state (**Status Color**). It draws **control signals only**; power wiring is up to the builder, and is described, never drawn (#293). Operator, 2026-09-19 (the Wiring design review): *"the wire lines in the drawing should initally say what pcb silkscreen label and then what we have now"*, and *"Do NOT use the term 'in use' that sounds weird for droid wiring"*.
_Avoid_: a hand-drawn diagram, a static image anywhere in the pipeline, drawing power distribution, a fallback board that is not in the droid, "in use" for an **Output** (say *wired*), **"lead"** for a wire (operator, 2026-09-20: one word, and it is *wire*), "driven"/"drives", "loom" as a heading (say *the wires*), a wire named only by protoArtoo's word when the board prints another


**Parts**:
The destination that answers *which **Output** moves this **Part*** and *what does this Output move* - the same mapping read from both ends, as two projections of one table rather than two surfaces that can disagree. It carries the part-first projection (grouped by class, with a count in each heading and a group for **Common Addition**s) and the droid picture. The output-first projection, its bulk actions (#318), the calibration dial (#291) and **Find by Moving** live on **Servos** (operator, 2026-09-19, #412: *"move bascially all of the "Outputs" section pieces to the "Servos" page. They all seem related to servo calibration"*). It sits in the **Configure** **Activity Group** beside **Wiring**, which answers the neighbouring question - *where does this wire go* - and sets nothing but which Outputs are wired. The **Droid Build** is stated at its head, because everything a Droid Build does is decide which Parts are offered and drawn, and #333 already made fitting a Part an act performed on this list (#288). Guided **Setup** could not host it: #297 made that one-shot, and this is a table a builder returns to every time a linkage is rebuilt or a servo moves to a spare output. The name is the model's head term in the plural on purpose - the page is where a builder meets every Part of their droid (#296).
_Avoid_: mapping page, assignment page, Setup, Bench (that word is already spent on **Bench-Mode** and the **Bench Runbook**)

**Servos**:
The destination for the body's **Output**s as servos: one section of Outputs, each named by what its board prints and by the **Part**(s) assigned to it. On each Output you pick its servo, drive it (Open, Close, Stop, or a typed width sent once), record its ends with the calibration dial, and find what it moves with **Find by Moving**; *centre all* sits over the section. It is the output-first side of the mapping that **Parts** reads from the part's end. There is no separate test section: testing a servo is driving its Output (operator, 2026-09-19, #412).
_Avoid_: "Test a servo" as a section of its own, "Servo on each output" (say *Servo assignment*), calibration on Parts

**Lights**:
The destination that answers *what lights my droid has, how each one is lit, and what I can tell it to do* - every **Part** whose **Part Kind** is a light, grouped by where it sits. It is not a page about any one lighting product, and an LED strip is a **Light Type** rather than a thing on the droid (operator, 2026-09-20: *"LED strip is simply one of the differed light types we can assign to a droid part"*). A light protoArtoo drives names its **Light Type** and its settings, and reads *on*, *off* and *flash* with brightness for *how far* - the words ADR 0049 gives every Part, so one word means one thing across the droid. A **dome light** names no Light Type, because the dome controller owns its hardware; it offers that controller's own modes and colors instead, under the labels a **Sequence** already shows, and what it reports is commanded intent and never device truth (ADR 0045). Which wire carries a light is **Wiring**'s answer, not this one. A row added to the **Droid Parts Catalog** appears here with no code change, and a light nobody has fitted still appears - *choreograph first, wire later*.
_Avoid_: LED page, "LED Strip" as a section, "not driven" for a **dome light** (the dome controller drives it and this surface commands it), naming an **Output** or a **Board Component Label** here, a control for hardware nothing drives

**Find by Moving**:
How a builder discovers which **Output** drives a **Part** when nothing on the droid is labelled: from **Servos** (it moved from an unwired Parts row, operator 2026-09-19, #412), protoArtoo steps through the unclaimed Outputs, nudges each one briefly, and records the assignment when the builder says *that one moved*. The nudge is **bounded and symmetric about where the Output already is** - never to an endpoint, and never to a recorded end that may not exist - because an unassigned Output is almost always an uncalibrated one, where the first move is a jump rather than a ramp. It is motion, but like every browser-initiated servo move it asks for no **Non-RC Control** consent - that flag has never reached `POST /api/servo` (ADR 0064). Any **Estop** ends the run, and a run nothing responds to ends with the Part still *- not wired -*. It is the calibration dial's own philosophy turned on the mapping problem: you do not know which output it is in advance, you find it by driving (#296).
_Avoid_: auto-map, auto-map by name (an **Output** carries no name to match against), wiggle-to-pick (the reference's term, and for choosing an RC device)

**Setup**:
The guided first-run wizard, and nothing else. It continues the flow WiFi Provisioning already starts - a fresh controller is unprovisioned and every component toggle is off, so the operator is walked from a connected controller to a declared droid in one numbered pass, every step skippable. Reaching the last step ends the run for good and Setup leaves the nav; Configuration is the surface an operator returns to for every later change. Maintenance carries the single deliberate way back in, for a builder who skipped it or rebuilt the droid wholesale (#297). *A declared droid* is literal: one of the steps asks the **Droid Build**, so the run ends with the droid's own parts known rather than only the components fitted to it. The run **ends by saying what it changed** - the answers given and the concrete effect of each, with the destinations now reachable - rather than with a farewell or a next instruction. Its rail carries the answered questions and nothing else: the second tier the reference puts there holds what it calls jobs, and ours became destinations with permanent addresses, so a second door to them inside a surface that leaves the nav would be the drift #288 closed (operator, 2026-09-08). WiFi Provisioning remains the separate first-boot networking term for the networking step itself.
...
*not fitted* is a **card** rather than a checkbox, because declining a category is an answer like any other, and it clears that category's Component Toggle. There is no *something else* card: nothing in the Component Registry can represent an unlisted part, so such a card would behave exactly as *not fitted*, which already says what still works (#369). A family whose one product is always present - the **Built-in servo outputs**, the Body Controller's own pins - carries no *not fitted* card, and the Body Controller family is shown, not chosen: the board this image runs on is what the builder has (operator, 2026-09-18). A roadmap card is **not a control at all** - static content, not a disabled button - which is what keeps it out of the availability state machine #288 excluded it from and makes "greyed but still clickable" impossible. There is no "stands in" card: saying an unlisted part will probably work through a protocol we support is a *might*, and #303 gave "might" no home. The product image is the card's main selection highlight, the thing an operator matches against the hardware in their hand; a card still missing its photo is a cosmetic gap and must never read as a part the board cannot take. **A line drawing is not that gap**: on the **legacy** asset set every card carries a drawing in place of a photograph, which is the normal and finished state there rather than something missing, and the card lays out the same either way (ADR 0065).
_Avoid_: a wizard-only lineup, a config-only lineup, two pickers, a separate checkbox for declining a category, a missing photo styled like an unavailable part, a *something else* card, a roadmap card rendered as a disabled control, a stands-in or probably-works badge
...
**Live Reading**:
What a surface is handed about the droid's live state: the last status frame that carried all six core fields, with each field answering heard, **Finding out** (no good frame yet) or **Unknown** (frames arrive but never carry it). One module owns it, loaded and started by the **Operator Shell**: the stream-or-poll choice (one 5 s poll for the whole shell when there is no stream), the frame check and both words. The **Status Plate** reads it the way every surface does. A frame missing a core field is ignored and the last reading stands. The estop is three-valued in it, and a move act is live only on a heard, clear estop: Finding out or Unknown keeps it off, and losing contact with the droid turns the estop back to Finding out while the other fields keep their last value. It covers the status frame only: an Output row's `reported` belongs to the Output rows, and what a page has told the dome is that page's own record (#419).
_Avoid_: a surface reading `payload.estop` itself, a per-surface stream-or-poll block, a per-surface unknown word, a missing field read as the other value
...
**protoR2link**:
The dome-body link subsystem. The canonical operator-facing name for the connection between the body controller and the dome controller. Used in the web UI component label, the **Status Plate** chip and task/issue language (operator, 2026-09-25, #422).
...
The active transport (UART slip ring or WiFi fallback) must be surfaced to the operator in the main dashboard dome status badge and the protoR2link component panel. While linked, the transport **is** the protoR2link **health signal**'s value - its operator label, and `UART` / `WIFI` on the **Status Plate** - rather than a separate word beside it (#422). The transport field is available in the `/api/status` `dome_link.transport` response field.
...
The body-side owner of DM:* choreography. It applies lookup precedence, advances the timing cursor, and dispatches each step; dome panel steps use calibrated panel-intent commands rather than raw servo-slot pulses. It is also the only thing that expands a body **Gesture** or a body flutter into individual **Output** moves, and the only thing that holds the **Cadence Floor** between them - which is what keeps a safe pace out of the browser, where a hand-edited or imported sequence could walk around it (#331, #319).
_Avoid_: dome sequencer, raw-pulse panel choreography

**Command Ownership**:
What decides whether the body answers a community Marcduino command itself or forwards it to the dome: **it owns a command when the command names a **Part** the **Droid Parts Catalog** does not mark `control: dome-link`**. Prefix does not decide it - the body used to consume every `:` and `#` and report success whether it acted or not, which is why 23 commands the dome fork implements were unreachable. Ownership is derived, never declared twice: the catalog is already the one generated declaration (#301), so a routing table or a per-command field would be a second home for a fact that has one. A command naming a Part the body models but no **Output** drives stays the body's and reports `part-not-assigned` at run rather than being forwarded into silence. What protoArtoo answers is published; what it forwards it says only that it forwards, because the dialect has no reply channel and what the dome does with it is the dome's to report (#321, ADR 0055).
_Avoid_: prefix routing, a hand-kept token list, claiming to know what a forwarded command did, accepting a command and reporting success without acting
...
**Reaction**:
Something the droid does because it noticed something, rather than because anyone fired it. It is a **trigger binding whose source is a droid condition** instead of a radio channel - the action half, the action registry and every **Sequence** are unchanged, so a builder authors a Reaction where they already bind switches, in a vocabulary they already know. What is new is what may be a source: what the droid is *doing* (drive speed and steer at 50 Hz, a hard stop, going stationary, a track starting) and what its drive *feels* (per-wheel speed and current, so a droid being shoved or held knows it). A source that needs drive feedback reports its own **Availability Family** on a drive that cannot provide it, rather than silently never firing. **What the droid cannot perceive is the room**: there is no proximity sensor and no IMU on any protoArtoo droid, and the audio path reports `playState` and `currentTrack` but never a level - so a light can change when a sound starts and can never follow one. Each Reaction carries how long it stays quiet after firing, and one droid-wide floor keeps a burst of triggers from stacking into a chorus. **While the droid is driving a Reaction may light, sound and turn the dome, and may not drive a body Part open** (#332).
_Avoid_: reflex, autonomous behaviour, a reaction as a second kind of sequence, audio-reactive lighting, a reaction that fires silently on a droid that cannot sense its trigger
...
**Sound Bed**:
A track that plays **under** what else the droid is saying - music beneath a performance - carrying its own volume, which a vocal fires over without stopping. It is a **Sequence** step like any other sound: a sequence starts a bed, fires over it and stops it, so a show is one authored thing a builder can re-tune rather than a live juggling act, and a **Sequence Tempo** finally has music on the droid to line up against. It exists **where the fitted module mixes** - the CHIRP Audio Trigger is a multi-stream board carrying three or more independent streams, while the DY-SV5W and the SparkFun MP3 Trigger are the single-track modules it contrasts itself with (`docs/sound_playback.md`). So it is a **Component Family** capability like every other: a bed authored for a module that cannot mix is a **Rehearsal Warning** and a run-time report, and the sequence still saves and still runs without it (#326).
_Avoid_: background music as a separate subsystem, a bed as a droid-level mode, assuming every sound module can mix, a bed that a vocal has to stop

**Named Track (authoring)**:
A canonical, config-backed sound role (the AudioNamedTracks namespace, for example scream, leia, cantina) used as the authoring surface for a sequence's sound steps, so a sequence references a sound by role and follows the operator's configured track number. The Marcduino $NNN and $-letter dialect stays valid at command boundaries for interoperability. **Two layers, doing different jobs.** The fitted module's own address is shown the way the builder arranged it - bank, page and index on CHIRP, `NNN` inside its community category range on a flat module - because that structure is a thing they physically built and can see on their own card. A Named Track sits over it as the portable half, so a sequence survives a module swap or an SD reshuffle. A name read from the module *describes what is on the card*; a Named Track *names a role*. Neither overrules the other because they are not the same claim, and the one real collision - a Named Track pointing at an index whose file changed underneath it - is a **reconciliation the builder resolves**, never a silent re-point (#326; the rule #333 gave a dome reporting a layout that disagrees).
...
**Learned Sequence**:
A DM:* sequence defined as a JSON file on the controller filesystem (/seq/), created or edited without reflashing, accepted only after passing Protocol Check, and executed by the same coordinator engine as a Factory Sequence. How many a droid stores is a board fact the droid reports: **five on the artoo-esp32, ten elsewhere**, because there the web image and the sequences share one small partition (ADR 0065, amended 2026-09-25).
...
**Timeline**:
The horizontal drawing of a **Sequence** as time — a lane per **Part**, blocks placed by when each step runs, and a marker the builder moves to any moment. It draws the routine the builder wrote, never the commands it becomes; the expansion sits beside it as a second reading of the same steps (ADR 0057). A block is a **span derived from the steps**, an open paired with its close, so a part opened and never closed draws for as long as its own travel takes and then reads as still open when the routine ends — which is how a builder sees what they are walking away from. Its beat grid comes only from a tempo stored on the Sequence and is never inferred. Parts the routine names that no **Output** claims are said once above it rather than marked lane by lane, because on a droid mid-build most parts are undriven and *author before you build* must not read as a screen full of faults. The first one ships **read-only**: it moves the droid only when asked, one pose at a time, and never follows a dragging marker (ADR 0062).
_Avoid_: `scrubber`, a timeline drawn from the expanded commands, a beat grid inferred from the spacing of steps, the droid following a dragging marker, a lane that is not a Part
...
**Rehearsal**:
The advisory pass over a Learned Sequence at the moment it is committed, which reads the sequence and says what will not happen as its author wrote it. It can never refuse a save, which is what keeps "which one said no" from ever being a question: **Protocol Check** is the only gate. Named for what a sequence is -- a performance -- rather than for the mechanism, and deliberately not a second "Check" (#287, ADR 0044). Its rules are admitted by one standard: **a rule is paid for by a failure this project has had**, because a rule nobody has been bitten by trains an operator to scroll past the list. Another project's bench does not pay for ours, so a rule derived from a reference catalogue and never matched to an incident here does not ship - which is what cuts the near-the-caps and nothing-fires-it rules an earlier pass had called settled, neither having bitten anyone in the years they had the chance. It reads a **Learned Sequence** at save, and a **Factory Sequence** at the moment it is cloned - the point where a factory routine's defects pass into a builder's own work. A Factory Sequence nobody clones is never read, and there is deliberately no build-time checker over the catalog (operator, 2026-09-08).
_Avoid_: linter, pre-flight check, sequence lint, validation pass, dry run (nothing is driven), a rule ported from another project's catalogue
...
**Body View**:
The drawing of the droid a builder reads state from — a recognisable R2-D2, the Star Wars astromech, front and rear, with each part drawn where it sits. **The features drawn follow the builder's design origin**, the **Body Design** and its **Design Variant** (MrBaddeley MK4 Complex or Basic), so it never shows a part that design does not have (ADR 0063, amended 2026-09-18). Its job is showing **many parts at once**, which a named list cannot do; finding a part is not its job, because a body part's name already carries its location in a way a dome part's number never does. It shows **one kind of state at a time and says which** — what the droid was last told, or what a **Sequence** says at a chosen moment — because both are commanded intent and would otherwise be indistinguishable. A **click only ever selects**: the acts available on the picked Part are named beside the drawing, which is also where its explanation lives, visible rather than on hover (ADR 0059). A part can be **moved from it on a deliberate press** - run through its travel and back as one command - and like every other browser-initiated servo move it asks for **no Non-RC Control consent** (ADR 0063, corrected 2026-09-09; ADR 0064). That flag has never reached `POST /api/servo`. It is one card with three faces - **Front**, **Rear** and **Dome (Top)** - the dome kept top-down because an elevation cannot fit every panel. An open Part fills with the accent, as the dome picker always has, and closed spends no color. The legend is a fixed list of seven one-word states (Closed, Open, Unmeasured, Limp, Unassigned, Unfitted, Picked), always shown. The "Other part" slots are not drawn and not listed beside it: they stay names for off-model hardware on the Parts table (ADR 0063, amended 2026-09-19, #408).
_Avoid_: an abstract layout of boxes standing in for the droid, drawing parts this droid's Body Design and Design Variant do not have, color standing for what a Part is, live state and a Sequence's moment drawn together, a click that adds or assigns, placing your own parts on the drawing
...
**Part Kind**:
What a **Part** usually is — servo-driven, a light or display, an indicator. Successor to the catalog's `lit:` annotation, and it has replaced it: the notes on six lit dome panels are six light Parts declaring `kind: light`, each linked by `sits_on:` to the panel that carries it (#357). It is **advisory in principle, never a constraint**: it gives a Part its own treatment on a surface and lets one query a surprising mapping, and it never refuses one, because plenty of builds move something the reference drawing shows as a display (#320). A Part with no declared Kind is one the catalog does not classify — the escape-hatch slots are whatever the builder wired to them — which is not the same statement as classifying it as something that moves. The treatment **removes** what the Kind cannot promise: a light has no travel, throw, position or **Output Release**, so a surface omits them rather than showing them as zero, since a zero still reads as a promise about movement. A light Part does not carry how it is lit - that is its wire's **Light Type**, and a light on no wire of ours has none.
_Avoid_: part type, part class, component type (that one names what is fitted to an **Output**, not what the Part is)

**Light Type**:
What protoArtoo puts on the end of one of its own leads to light a **Part** - *LED strip* today, and a list that grows. It is the light half of the same answer a servo gives as its model, because both say what is physically on that wire; it is therefore a property of the **Output**, and a **Part** inherits the type of the wire it is on. **A dome light has no Light Type**: the dome controller owns its hardware, and protoArtoo reaches it by forwarding documented commands (ADR 0045). A droid may have **several lit body Parts, each on its own wire** with its own type and settings (operator, 2026-09-20).
_Avoid_: light kind (that is **Part Kind**), LED type, a lighting product as a type (*AstroPixels* and *Teeces* name a product a builder fits, not what is on our wire), "AUX LED", a Light Type on a **dome light**

**Output**:
An addressed row that drives one or more **Part**s: an **Output Address**, the Parts it drives, and a **kind** saying what is on the end of the wire. Usually one - but a wire ganged to two parts that always move together names both, so nothing physically driven is invisible to the software; two Parts on one Output cannot move independently, which is a **Rehearsal Warning** rather than a refusal (#296). Rows are mixed by design — a servo, an RGB strip and an indicator chain sit in one table — because an Output Address is where a wire plugs in, whatever it plugs into. What is on the wire is one answer in two vocabularies: a servo's model where it drives a servo, a **Light Type** where it lights something. Kind decides which columns a row carries at all, so a light row is not a servo row with five meaningless fields (#320).
_Avoid_: channel, slot, Servo Output (that names one kind of Output), an Output that shares a Part with another Output

**Servo Output**:
An **Output** whose kind is servo: one physical servo the body drives, carrying - beyond every Output's address, Part and kind - its calibration (open/centre/close), its **Motion Profile**, its **Output Release**, its boot behaviour (limp, go home and hold, or go home and release - limp by default, and calibrating an Output never ticks it), component type, and a `calibrated` bit. The boot pass is generated, so the **Cadence Floor** paces it and twenty-three Outputs never start together (#286). Outputs are addressed rather than named so a body servo controller can add rows instead of forcing a rewrite; the model is deliberately not bounded by the current boards' pin budget (#286).
...
**Motion Profile**:
The three things a builder sets about how a **Servo Output** moves, and the answer to *what does this part feel like*. **Speed is a time**: how long a full throw takes, end to end. **Acceleration is a time too**: how long the move spends getting up to that speed. Both in milliseconds, because a time is the physical consequence a builder can check with a stopwatch, where a rate is something they have to convert into one - and mis-reading a rate as a time is the error the reference paid for on its own bench, where speed 80 with acceleration 10 reads as ~344 ms and is really ~940 ms. **Easing is the shape of the move, not its rate**: `none` stops dead on the number, `soft` eases the acceleration itself in so a long linkage or a heavy panel breathes into motion, and `overshoot` aims a little past the target and settles back, which is what makes a pie read as snapping open rather than arriving. **Overshoot never passes the recorded ends**, so on an Output whose `calibrated` bit is unset it has none to work within, degrades to `none`, and the surface says so. It lives on the Output, so a builder sets a heavy door's character once; only a **Gesture** overrides it (#286, #319). **It is the body's alone**: the dome firmware moves its own panels and holos, and the body reaches them only through the dome's existing commands (**Catalog Authority**), so a dome Part's feel is not a Motion Profile (#414).
_Avoid_: speed as a rate, a stored travel time beside a rate (one number, the other derived), easing as a kind of acceleration, overshoot bounded by the absolute pulse clamp

**Output Release**:
The bounded hold after a **Servo Output** reaches its target, after which its drive is cut so the servo stops holding position. It exists so a jammed, mis-wired or fought part cannot grind indefinitely under held drive. In normal operation the release is scheduled from **arrival**, not from when the command was issued, because the motion model runs in firmware and knows arrival exactly; any new command to that output cancels a release pending on it. **The calibration dial suppresses it** for the one Output it holds, because release fires exactly when the builder has stopped moving a Part in order to look at it - and a struggling servo is only audible while it is being driven. Two firmware bounds replace it there and the page can extend neither: a short expiry when commands for that Output stop arriving, and a ten-minute ceiling from when the dial took it. Either one leaves the Output limp and says so, and one press resumes (ADR 0064). Why an Output is limp is said in **one set of words wherever it is shown** - Servos, Parts and every other surface read it from the one reader of an Output's live state, never from a table of their own (#421). **Estop and Sleep Mode release every output at once and command no position**: driving many outputs together is the documented brownout path (the 2026-06-17/-18 fix; `src/tasks/sequence_catalog.cpp:205-212`), and a browned-out board drops the parts anyway and clears the toggle latches as it goes. A released output is limp, not moved — release says nothing about where the part ends up. **It is a Servo Output's field and a light row does not carry it**: release exists so a jammed, mis-wired or fought part cannot grind, a light can fight nothing, and stretching a hazard control over a device with no hazard would put a word on a row where it means nothing - **kind decides which columns a row carries at all** (#320, #318). What **Estop** does to a *light* is therefore a separate question and an open one: the engine resets dome lights at sequence end but not on estop-clear, so a latched estop leaves the holos running today (#320).
_Avoid_: sleep-when-idle, sleep (that names a droid-wide mode), detach, park, torque off, a per-surface word for why an Output is limp

**Output Address**:
Where an **Output**'s wire physically plugs in: `(driver, channel)` — for example LEDC channel 3, or board 1 pin 4 on an expander. It is wiring, not identity; moving a servo to a different address must never change what a sequence means. **An Output is called by what the running board prints beside its pin** - its **Board Component Label** - on every screen, in the Console's list and completion, and as the word typed in the Console and sent to the API: `ARM1`..`ARM5` on artoo-esp32, `GPIO 49` / `GPIO 50` / `GPIO 4` / `GPIO 5` / `GPIO 51` on firebeetle2, where the IO shield prints only the number. There are no protoArtoo-wide output names and no split into "arm" and "AUX" kinds: all of them are **Output**s, and whether one can carry the LED strip is a fact about that Output on that board. `arm1`..`aux3` survive only as hidden ids in saved settings and config keys. Sequences store **Part**s, so no sequence names an Output. Operator, 2026-09-19: *"the AUX and ARM labeling are pure Artoo PCB specifics and not at all relevant to firebeetle and future boards, hence that should not be hardcoded"*.
...
**Droid Parts Catalog**:
`docs/droid-parts.yaml` - the one declaration of every **Part** on the droid design and its **Part Kind**, and the source both the firmware and the browser are generated from rather than a document either reads at runtime. Every entry it declares reaches both generated outputs, so the compiled id the **Protocol Check** vocabulary gate accepts is the whole catalog: a builder who wires a spare output to a breadpan door records that door's own id rather than an escape-hatch slot (operator decision 2026-09-11, #358). Its `control` column says what is DRIVABLE and never what is NAMEABLE - the dome still owns execution of panel intent under **Catalog Authority**, which decides who drives a Part, not who may name one. Entries are build-time; `other1`..`other10` are the escape hatch so an off-model part wired to a spare output is never unnameable (#301). Its `control` column also decides **Command Ownership** - a community command naming a Part not marked `dome-link` is the body's to answer, and everything else is forwarded (#321).
_Avoid_: a runtime-editable parts file, a firmware table hand-maintained beside the YAML, two catalogs for one droid

**Part**:
What the droid is made of, part by part — a utility arm, a charge bay door, a dome pie, a PSI, the Magic Panel — identified by its key in the **Droid Parts Catalog** and carrying a **Part Kind**. A light is a Part exactly as a panel is, and a device that both moves and lights is several Parts, one per thing an **Output** drives — a holoprojector is a pan Part, a tilt Part and a light Part (#320). Sequences and RC bindings reference the Part; an **Output** records which Part it drives. Part names are identity; the **Output Address** is where the wire plugs in. A Part being *known* and a Part being *driveable here* are separate facts: naming one no output records is legal to author and reports `part-not-assigned` when run, so the droid's own wiring - not the catalog - decides what moves (#301). A Part is driven by **at most one Output**: assigning it to another Output *moves* it rather than sharing it, which makes the conflict unrepresentable instead of merely reported. **A move is announced before it happens, on every surface that can make one** - the part-first table, the output-first table, guided **Setup** and any import alike - naming the Part, the Output it leaves, what that Output keeps and what the Part will move with, and asking with the verb. The firmware refuses a move that does not name the Output the Part is leaving, so no surface can take a Part off an Output without having read it there first, which is the moment it has what it needs to ask. Choosing *- not wired -* on a Part's own row, and putting an unwired Part on an Output, take nothing from anywhere and are not asked (#347). Operator surfaces label a Part with its plain-English alias - *Left body door*, *Dome pie 1* - with the Printed Droid shorthand alongside on dome parts, and the `cad_name` reaching the bench through **Wiring**, the declared bridge between the two naming systems. A builder does not rename a Part, so a screenshot, a support request and a shared sequence read the same on every droid (#296).
_Avoid_: channel, actuator, output (an output drives a Part, it is not one), treating an unclaimed Part as an authoring error, a per-droid rename, a CAD name as the operator label

**Droid Build**:
Everything protoArtoo knows about which droid it is bolted into: a **Dome Design** and a **Body Design**, each with a **Design Variant**, together with the **Fitted Parts** they seeded and any **Common Addition** the builder added. It is what a builder means by "my build". A Droid Build **seeds and never fences** — it decides what a builder is offered and what the mapping views draw, and it never decides what they may author, assign or run. Stored on the device like any other operator answer, so a second browser or a cleared cache meets the same droid; no firmware logic branches on it (ADR 0047, #333). Stated at the head of **Parts**, the surface its answer changes (#288), and asked once during guided **Setup** - one step carrying both designs and both variants, rendering the same builder that Parts renders, so the two homes cannot drift; it is the same one-builder-two-homes rule the **Component Picker** already follows (#297).
...
**Body Design**:
The published droid design the builder's body was built from, answered separately from the **Dome Design** because a real droid is a mixture — an MK4 complex dome on an MK4 basic body is the ordinary case, not an edge case. It seeds the body half of the **Fitted Parts** and decides what the body views draw. Unlike the dome half it has no second authority: nothing reports a body complement back, so the builder's answer stands alone (#333).
_Avoid_: droid design as a single value, body model

**Design Variant**:
The second field beside a **Dome Design** or **Body Design** — *basic* or *complex* on the designs that publish both, in MrBaddeley's own words (MK4's sparse variant was misnamed *simple* here until 2026-09-18, and a build stored under that spelling still reads as *basic*, #409). Each design declares its own variant set, which may be empty, so the control appears, repopulates and disappears with the design chosen rather than offering one fixed axis everywhere. A variant is not cosmetic: it decides which complement is drawn, and a basic dome cannot grow the complex pies without being replaced, so drawing the maximal complement at a simple-dome builder would promise parts they can never fit (#333).
...
**Common Addition**:
A **Part** in the **Droid Parts Catalog** that belongs to no design and is offered to every droid — the two utility arms, the gripper arm and its claw, the interface arm and its tool, the things builders bolt on. Never seeded by a **Dome Design** or **Body Design**; once fitted it draws and behaves exactly as a design part does, carrying its own bearing, position word, name and lane. It is not the `other1`..`other10` escape hatch: that exists for a part we have no word for, while a Common Addition is one we do. All six body arms are Common Additions, never fitted by default: `utilUp` and `utilLo` (which carry MrBaddeley CAD names but are optional on every MK4 build, operator's verified mapping 2026-09-18, #409), and `gripArm`, `gripClaw`, `interArm`, `interTool`, which carry `cad_name: null` (#333).
...
**Sequence Tempo**:
The beat a sequence is written against, so a step or a **Gesture** can be placed on a beat rather than at a millisecond - a wave one Part per beat, pies alternating every two - and so a whole routine can be retimed by changing one number. A tempo read off a track is **advisory and always editable**, never a fact: the analysis folds metric levels, and the specimen that proves it is Cantina, whose ~200 BPM reads as 127.8. Beats resolve to absolute milliseconds before the **Sequence Coordinator** schedules anything, so nothing about run-time timing becomes less deterministic than it is today (#331). It comes from one of three routes and **says which**: the builder types it, drops their own copy of the track in to be analysed, or **taps along to the track playing on the droid** - the route that always works, because it needs no file and is the only one that uses the audio protoArtoo actually has. Every route carries a **confidence** - the analyser's own figure, or the spread between taps - and a weak one is a **Rehearsal Warning** rather than a refusal, since an analyser that can say no is the authority this term refuses to make it. The sequence keeps the tempo, its source, its confidence and a hash of what was analysed; a step placed on a beat keeps **the beat beside the millisecond**, so retiming re-resolves rather than rewrites and nothing needs undoing. A hash that stops matching is a Rehearsal Warning too - the audio behind a sequence can change with the sequence untouched, because a sound is named as a role and the operator's config supplies the number (ADR 0058). A builder puts a step on a beat by **picking the beat**, not by dragging until something lands near one - the sequence already stores the beat, so picking is exact and needs no pointer, which is what a bench needs. **A move's duration may be a span of beats as well as its start**, so a move can fill a bar; what is stored is still a time (ADR 0052), and a tempo raised until the part cannot keep up is a **Rehearsal Warning** on the slowest throw, never a refusal. **Where bar 1 starts is set by the builder** - a tapped tempo's first tap is it - because a downbeat guessed from onsets is the part of this that measurably does not work (ADR 0060).
_Avoid_: BPM as a measured property of a track, tempo as authority, beats surviving into the engine's schedule, a detected tempo that cannot say how sure it was, a beat-placed step stored only as a millisecond, a snap mode that restricts which beats may be picked, a snap tolerance measured in screen pixels

**Gesture**:
One authored move spread across several **Part**s - a wave round the ring, both breadpan doors alternating, six pies shaking - carrying a shape, the Parts it spreads across, and how it spreads across them. It is authored as one thing so it can be re-tuned as one thing: a builder never writes the per-part offsets, which is what a **Learned Sequence** costs today, `DM:CANTINA` spending 26 steps on a two-beat alternation and `DM:RESET` seven on one safe close. Its order comes from where the Parts physically are, not from the order they were listed, and it keeps meaning what the builder said rather than the parts that existed the day they said it: "the ring, clockwise" is worked out against the droid as it is when it runs, so fitting the eighth panel puts it in every Gesture that names the ring with nothing re-authored. A Gesture is two independent choices - what each Part does, and how the move travels across the set - with speed, easing and how far each Part travels as parameters of the Gesture rather than baked into the name of an effect. **Coordinator Resolution** decides who performs it: a dome Gesture resolves to what the connected dome can execute under **Catalog Authority**, and a body Gesture is expanded by the **Sequence Coordinator**, which is the only thing that can hold a safe cadence there. What a Gesture can express is set by what a builder needs to say, never by the command set a particular dome firmware happens to ship today (#331). A Gesture may also carry **how often it repeats and how far**, which is how a builder turns a track into a routine: point the shape at a group, say every how many beats, and it runs the length of the track. That is deliberately not a generator - it produces **one editable object** rather than a routine to take apart, because editing the expansion is what ADR 0057 refused and what ADR 0046 exists to prevent (ADR 0060).
_Avoid_: macro, group command, brick, a fixed list of panels, defining the vocabulary from the dome's current commands

**Body Step**:
The step that moves one body **Part**: the Part, a **Move Shape**, how far it goes as a fraction of that Part's own throw, and - for a flutter - how long it goes on. It is its own step type rather than a body target smuggled into a dome command's payload, on the precedent `STEP_DOME_ROTATE` set for a body-owned motion step. Speed, acceleration and easing are not on it - those live on the **Output**, and only a **Gesture** overrides them, so a builder's choreography travels and their physics does not (#286). Naming a Part no **Output** claims is legal to author and reports `part-not-assigned` at run (#301). A Part left open when the sequence ends stays open: the engine undoes nothing, and the author writes the close exactly as they already must for pie panels (#319).
_Avoid_: servo step (a body Part may be a light), body command, arm step, raw pulse step

**Move Shape**:
What a step or a **Gesture** says one **Part** does: **open**, **close** or **flutter** - the same three the dome's **Panel Intent Command** already carries, so one word means one thing across the whole droid. The surface names them by **Part Kind**: a servo Part reads open/close/flutter and a light Part reads on/off/flash, while the stored shape is one token either way, which is what lets a Gesture spread one shape across a mixed set. "How far" reads as travel on a servo Part and as brightness on a light. A body flutter is performed by the **Sequence Coordinator** rather than by the dome; like a dome flutter it ends open and owes a later close in the same branch, which is a rule **Protocol Check** already enforces (#319, #320).
_Avoid_: motion type, block shape, effect, a second vocabulary for lights

**Cadence Floor**:
The minimum spacing the **Sequence Coordinator** holds between body **Output**s it starts, when it expands a **Gesture** or a flutter. It exists because overlapping servo inrush is what browns a controller out. It applies only to moves the body generated: steps an author wrote by hand keep their timing and earn a **Rehearsal Warning** when they overlap, because pacing what you generated is not rewriting what somebody wrote. It also paces a **bulk motion** an operator starts from the output table - *put every output back to centre* is asked for once and **expanded and paced by the body**, never stepped through by the browser, because keeping the safe pace out of the browser is the reason the Floor lives in firmware at all. That is why a bulk motion is not the many-at-once shape ADR 0043 refused for estop and ADR 0049 refused for terminal cleanup: those are emitted automatically with nobody deciding, and this one is chosen by an operator standing at the droid (#318). The number is settable and **unmeasured on the body** - the 2026-06-17 brownout was measured on the *dome* controller, whose seven ring servos share the dome supply, and the body has never browned out - so the default is a stated conservative guess awaiting a bench run, never a dome figure quietly adopted as body truth (#319; the same answer #318 owes bulk actions on the output table).
_Avoid_: the ~450 ms cadence (that figure is the dome's), a fixed firmware constant, pacing hand-authored steps
...
**Setting**:
One value the droid stores that a builder can change - the top speed, how long the dome pauses between idle turns, an **Output**'s time to full throw or its LED count. A Setting belongs to the droid or to one Output, and an Output carries its own. Each Setting is declared **once**, and every door that reads or writes it - a page, a restore, the **Controller Console** - goes through that one declaration, so one door cannot take a value another refuses, and a Setting that takes words takes the same words everywhere. What a Setting accepts is ruled on by the droid, once; a page never keeps its own copy of the range. A refused value reaches the page in the builder's words and with its unit, from one table that must have words for every Setting, so a wire name has no path to a screen (ADR 0059). A rule that spans Settings - three speed presets that must differ, an end kept inside its band - sits beside them rather than inside one. An act on an Output (capture an end, swap the ends, move a Part) is not a Setting, and neither is a **Record**; both still declare the fields they can refuse, so the one table words them too. The same table's entry is where every page takes a Setting's label and when it takes effect; the firmware declares the timing and the check holds the two equal (ADR 0068, amended 2026-09-26, twice).
_Avoid_: config key, param, option, a page-side copy of a range, a wire name in a refusal, a page's own label or timing for a Setting

**Record**:
Values the droid stores that the builder, or guided **Setup**, states together and that only mean something together - the **Droid Build** (both designs, their variants, the **Fitted Parts**) and guided Setup's record of where its run stands and which steps the builder was shown. A Record is not a **Setting**: its fields carry rules that span them (a design and its variant arrive together, the Fitted Parts arrive whole), and nothing downstream is gated on it. Each Record is one module that owns its fields end to end - their check, answering field, reason and accepts; their merge onto the **Working Snapshot**; their GET answer; their NVS save and load and when they are saved - behind one interface every Record shares, so the **Apply Core**, the **Commit Step** and GET loop over the Records as they loop over the Settings (ADR 0068, second amendment of 2026-09-26, #432).
_Avoid_: a Record field treated as a Setting, a Record checked or merged by hand at each door, **Console Record** (one line the Console answers with, unrelated)

**Apply Core**:
A pure module behind a write path: it reads parameters through a Param Source (a function-pointer name lookup), validates and applies them onto a working snapshot, and returns field-level errors, an applied-fields record, and plain-data actions. A refusal carries its **field, reason and accepts as data** beside its sentence, and every adapter answers from the data: no adapter, and no page, reads the sentence to find which field failed or what it would take (#425). Its side effects live in one Commit Step shared by every adapter; the HTTP handler and the Controller Console are transport adapters over the pair and own only their own response rendering (ADR 0011, amended by ADR 0036). The write-path counterpart of the pure GET JSON builders; cores: `api_config_apply`, `api_rc_map_apply`, `api_audio_apply`.
_Avoid_: handler helper, inline lambda validation, validation util, handler-owned side effects, an adapter parsing a refusal's sentence
...
_Avoid_: compile-time component toggle, feature flag, capability as a runtime setting, runtime-ready signal, a narrowed set standing in for a flash budget (a Gate states what the board can be wired for; what fits in the image is a different tier's answer, ADR 0042 amended 2026-09-09)
...
**Build Feature Flag**:
A compile-time `PA_*` flag, chosen per build environment and always defined as 0 or 1, declaring whether this firmware image was built with an optional feature such as the heap profiler. It is a developer choice about the image — not a fact about the board, and not an operator setting; a feature built out is absent from the image, not merely off. **It answers "is it in this image" for everything the image carries, files included**: a picture that ships to one build environment and not another is this tier's answer and not a fifth one, which is what the **Board Capability Gate**'s own `_Avoid_` line already pointed at — *what fits in the image is a different tier's answer* (ADR 0065). Each file declares which build environments carry it, so the fact sits beside the file rather than in a list that goes stale.
_Avoid_: build-stripping flag, developer toggle, compile-time component toggle, feature flag (unqualified), a per-board list of files kept apart from the files themselves

**Asset Set**:
Which pictures a build environment's filesystem image carries. Two exist: the **default** set, which carries photographs, and the **legacy** set, which carries a line drawing in place of each photograph. **Only what is shown differs** — the surfaces, the choices and the behaviour are identical on every board, and nothing a builder can do depends on which set their image holds. Which set an image carries is the **Build Feature Flag**'s answer; the name describes what the set holds and never the board that carries it (ADR 0065).
_Avoid_: legacy board, a reduced feature set, a cut-down image, a drawing described as a missing photo, an asset set deciding what a droid can do

**Component Family**:
A category of interchangeable hardware a droid can be built with — sound modules, drive controllers, servo controllers — whose members are reached through one interface and asked what they support rather than identified by name. Sound is the first and so far only one: three modules behind a common interface with a capability bitmask. A family exists for a category wherever the project supports more than one product in it — a project fact, not a board one. Whether a given board can be wired for all of them, whether a given image carries them, and whether the operator therefore gets a **Component Member** setting are three later questions (ADR 0042 as amended 2026-09-09).
_Avoid_: driver class, backend type, component category (unqualified), plugin

**Component Member**:
The runtime setting naming which member of a Component Family is fitted — **the product the builder actually bought**, which is what the picker shows and what the setting stores. It sits beside the Component Toggle and answers a different question: the toggle says the subsystem is fitted, the member says which product it is. Firmware reaches it through a **Component Protocol**, which is a separate question again. A member setting exists only where identity reports **more than one selectable member** — where one part is the only thing this image can drive there is nothing to choose and no setting. That is deliberately not the **Board Capability Gate**'s set: the Gate answers what the board can be wired for, and what the running image carries is a different question the identity manifest reports separately (ADR 0042 as amended 2026-09-09). Like a toggle, a change is staged at reboot.
_Avoid_: component variant, driver selector, board capability (a member is an operator choice, a capability is a board fact), model, a Gate set standing in for what the image carries
...
**Component Registry**:
The single declaration of every **Component Family** and its members: what the project intends to support, which of those are built today, which **Component Protocol** each member is reached through, what each can be asked, and how a board labels it. Firmware tables, the identity manifest and the operator lineup all derive from this one declaration, and a drift check reports where they have diverged rather than rewriting them — the convention the action registry set. **Every row reaches the image, including a part nothing drives**, carrying its own operator-visible name and its own capabilities; drivers are carried only for the parts built today, so a row present with no driver is the normal way an unbuilt part exists rather than a defect. The browser reads the lineup from the identity manifest and keeps the `data/` copy only as a fallback that says it is one, because firmware and web assets are uploaded separately and a controller can report a part its own web assets have never heard of (ADR 0042 as amended 2026-09-09). Declaring a part here is what makes adding one cheap; the deeper research and implementation for a given part live in that part's own ticket.
_Avoid_: action registry (that one declares Operations), parts catalog (that one declares Parts — what moves on the droid, not what drives it), codegen (the check reports mismatches, it never rewrites a file), a roadmap list the web owns on its own, capabilities declared per family or per protocol rather than per part
...
**Dome Bearing**:
Where the dome **believes** it is pointing, as an angle from the droid's own front - never a heading in the room, which protoArtoo has no sensor for. It is integrated from commanded turns against a calibrated full-turn time, so it is presented as believed and never as measured: any manual turn, any coast, any **Estop** or **Sleep Mode** leaves it **unknown** until the builder turns the dome to front and says so. Home is front, the orientation `docs/droid-parts.yaml` already anchors (`bearing_deg`, 0 = dead astern, 180 = dead ahead). A sequence may address it, beside the timed speed-and-duration turn that already exists, because the two are different contracts: a duration always completes and a bearing may not. A turn authored while the bearing is unknown is a **Rehearsal Warning** and a run-time report, never a refused save. An index sensor would **correct** this model rather than replace it, so it stays a roadmap item and not a fork (#322). The dome drawing may be **turned by the builder** to match how they are standing, and that angle is **view-only**: it describes the viewer, never the droid, so it never sets or reads the bearing and is held per browser rather than on the controller. One angle serves every dome drawing, so two surfaces can never disagree about which way the dome faces (ADR 0051 as amended 2026-09-09).
_Avoid_: dome position as a measured fact, a heading in the room, a bearing that survives an estop, dome angle, bare "bearing", the drawing's rotation as a claim about where the dome points, a rotation held per surface
...
**"Seated" and "unseated" are not part of this posture and are not used to describe it** (operator, 2026-09-17). They are an **artoo-esp32** fact about where its ESP32 module sits: in the Artoo PCB's header, USB flash and USB read fail because GPIO15 is shared with SBUS strapping (`AGENTS.md` "Seated controller: OTA + HTTP only"). That constraint is real and stays written where it belongs - the artoo-esp32 board notes, `docs/console.md` and `docs/troubleshooting.md`. It says **nothing** about a **firebeetle2**, which is an ESP32-P4 development board that is not seated into anything, and applying the word there implies a limitation that board does not have. **Bench-Mode already carries the whole meaning** - nothing connected - and it carries it on every Board Variant.
_Avoid_: bench verified, bench tested, bench-attachable peripherals, scoping pin-level electrical checks as bench work, treating an exceptional measurement as a routine one, using bench work as integrated-hardware evidence, **seated** or **unseated** as a testing or board-readiness posture, "unseated bench"
...
**Every no names the builder's next move, and takes them there.** A "no" that says only what is true has not earned its place: it costs a builder the evening they spend finding out what to do about it. So the three actionable families name the move *and* route to it - the same contract a **Status Plate** chip carries, *a route, not a control* (#324) - while **settled no** says plainly that there is nothing to do and stops, which is why it is the one family with no destination and therefore no link. A settled no never offers a date, a version or another product: pointing a builder at hardware to buy is a **Builder Recommendation** and is kept apart from every surface that merely reports what this droid can do. Naming a move is not enough on its own - measured 2026-09-09, the strings that named one were the ones that stranded a builder: 16 sent them to **Setup**, which is one-shot and leaves the nav, and 15 more named an `S1`/`S2` header that does not exist on firebeetle2 - so a move must resolve to a destination that is actually there (#298).
_Avoid_: one look for everything unavailable, a treatment per state, grouping by what is true in the firmware rather than by what the operator does next, a no that states only what is true, naming a destination the builder cannot reach, a date or a version on a settled no

**Health Signal**:
A reading of how one thing on the droid is doing right now, shown as a lit indicator beside its name - the estop, drive, the RC link, **protoR2link** and the sound link, the file system, memory, a fitted module answering or not. It answers *how is this doing*, which is a different question from *why can you not do this* (an **Availability Family**, told apart by treatment and never by hue) and from *what did the builder set* (a chosen posture, which takes no color at all). It is read as a droid LED - the color **is** the reading - and **Status Color** fixes which four. A health signal reports what the controller last heard, so a thing never asked reads **grey rather than good**: a green meaning *we did not check* is the one failure this term exists to prevent (operator, 2026-09-16). A signal has **one word per state**, and every surface that shows it uses that word, the **Status Plate** setting it in capitals and taking the word's short form where it has one. The droid, not the page, says which state a signal is in: sound, when **protoR2link** holds their shared serial line, is *held* rather than failed and reads grey. protoR2link is never held by sound: it gives the line up only while it runs on WiFi and takes it back at will, so a protoR2link *lost* is always a real loss (#422).
_Avoid_: telemetry (a number on **Dashboard**, not a lit state), a health signal for a setting, green before anything has answered, a per-signal freshness marker where the **Status Plate** already carries one, a page working out a link's state from raw fields, a per-surface phrasing for the same state

**Status Color**:
A **health signal** reads as a droid LED and takes a real LED color; nothing else in the surface colors for state. Four colors, each answering *how is this doing right now*: **green** - nominal: clear, armed, linked, reporting; **amber** - degraded, and you can do something about it, which also covers an uncalibrated **Servo Output** before its first move, a **Rehearsal Warning**, and a Part in a sequence that no **Output** claims; **red** - stopped or refused: a latched estop, a save **Protocol Check** would not take; **grey** - not reporting, never asked, not fitted. A **chosen posture is not a health signal** and takes no color at all - a speed preset, a **Sleep Mode** state, a control mode is a value the builder set, and coloring it would make a setting read as a symptom (**Status Plate**, *values, not exceptions*). **Blue carries no state**: it is the interaction accent alone - selection, the row shown, the primary act, the focus ring - because one hue meaning both *you picked this* and *this is alive* is exactly the overload #327 existed to prevent. One exception, the operator's (2026-09-19, #408): on a droid picture an **open** Part fills with the accent, as the interactive dome always has - it is commanded position, not health. A second, also the operator's (2026-09-19, the Wiring design review): on the **Wiring** diagram **each wire takes its own color and line type**, to tell wires apart the way a real loom does - it names a wire, never a state, and its colors are `:root` tokens like every other. The palette is **dark only** and declared once in `data/style.css`, so there is no second palette to drift against; a color literal outside `:root` is a defect, which is what makes the rule true rather than aspirational (#327, amended by the operator 2026-09-16: *"for the health signals overall we should not limit to only the blue theme colors, instead use the typical red, green, grey, orange colors ... having it be like droid leds with actual colors actually make sense and is star wars theme correct"*).
_Avoid_: blue for a live or healthy state, color on a chosen posture, a second orange beside amber, amber on a state the builder cannot change, amber on a roadmap card, amber on `checking`, coloring an **Availability Family** (those are told apart by treatment, never by hue), a light theme, a color literal in a component
...
**Non-RC Control**:
The Commanded Mode by which an operator consents to a non-RC source - browser, Controller Console, sequence - commanding the droid. It is **not** a failsafe override: a latched failsafe layer holds the droid whatever consent says, and consent neither clears nor suppresses one. The clause that used to say otherwise was removed on 2026-09-08 (#292) - it described a mechanism the project has decided against, not merely one it had not built, and a decision ticket must not leave an unwanted behaviour standing as a target. It is not persisted: it boots off, so a controller that has just restarted has consented to nothing.
_Avoid_: web control, network authentication, console unlock, blanket gate, a failsafe override, a mode that survives a reboot
...
- A **Component Member** answers which product it is, independently of the three tiers above: the Gate says the board can be wired for a part, the flag what this image was built with, the toggle whether the subsystem is fitted, and the member which product fills it. A member setting exists only where the image offers more than one to choose between — never derived from the Gate's set, which answers wiring rather than what the image carries (ADR 0042, amended 2026-09-09).
- A **Component Registry** row exists for every part the project intends to support and reaches the image whether or not anything drives it, so a row present with no driver is the normal state of an unbuilt part; only the drivers are conditional. Which tier reports a supported part missing from a given image is decided per case, never fixed in advance (ADR 0042, amended 2026-09-09).
...
- "feature flag" was used loosely for all three tiers; resolved by naming them **Board Capability Gate**, **Build Feature Flag**, and **Component Toggle**, and never using "feature flag" unqualified. A fourth tier joined them (#302, 2026-09-07): a **Component Member** names which member of a **Component Family** is fitted, wherever the image offers more than one to choose between (#302 again, 2026-09-09 — the condition was first written against the board's gate, and the gate answers a different question). The four answer four different questions — can this board be wired for it, is it in this image, is it fitted, and which one is it — and none of them is a "feature flag". A fifth was proposed for *what a board has room to show* and refused (#382, 2026-09-11): a file that ships to one image and not another is **is it in this image**, so the **Build Feature Flag** widened to cover files rather than the ledger gaining a term — the same refusal recorded for `capability envelope` and for the pin value that became a **Board Lane** (ADR 0065).
...
- **Non-RC Control**'s name reaches wider than the thing does, and the gap is a live one rather than a wording preference (#292, 2026-09-08). The term says an operator consents to a non-RC source commanding the droid; what it actually gates is `POST /api/drive` and the action-test dispatch path, so it holds back playing a random sound while `POST /api/servo` carries no consent check at all and never has - nor do sequences or the dome. On the drive path it is additionally a **no-op in both directions**: it only permits when an SBUS failsafe layer is latched, and that same latch zeroes the output, so consent changes a refusal into a silent zero and never into motion. Resolved for now by **leaving the behaviour exactly as built** (operator, 2026-09-08) and recording the gap here, so the reach is a known fact rather than a surprise - and so the failsafe override is not re-proposed by the next reader of the definition. Three surfaces state the reach wrongly and are recorded on #292 rather than fixed here.
...
- "sequence" carried two head nouns on one page: `data/seq.html:17` showed the operator **"Learned Sequences"** while `data/shell.js:30` put **"Sequences"** in the nav - the one-concept-two-names finding this map borrowed from the reference's own UX review, live in our own code. Resolved by making **Sequence** the single operator noun whatever the thing's size: one move across eight panels, or a phrase built from three others, is a Sequence, because #331 deliberately made a sequence inside a sequence the same kind of thing and a second word would undo that. **Factory**, **Learned**, **Retrained** and **Migrated** demote to *provenance qualifiers* - they stay exact in this ledger and appear in copy only where provenance is the subject, never as the head noun on the page a builder authors on. A **Gesture** is a step, not a small Sequence (#288, 2026-09-08).
- "Parts" the destination sits one letter from **Part** the model term, and the pairing is deliberate: the plural is the page where a builder meets every Part of their droid, the singular is what the page is full of - the same relationship **Wiring** already has to wiring. The label it replaces, **Servos**, is retired: it was already wrong, because since #320 an **Output** may drive a light, so a page named for one kind of Output misnames itself (#288, #296, 2026-09-08).
- "bearing" names two things that differ by what moves. A **Part**'s `bearing_deg` in the **Droid Parts Catalog** is a *fixed* fact about where that part sits on the dome - PP3 at 30 degrees - and never changes. The **Dome Bearing** is where the whole dome is currently pointing relative to the droid's body, and changes every time it turns. Resolved by keeping `bearing_deg` for the part and **Dome Bearing** for the dome, never a bare "bearing" for either. The two meet in exactly one place: a step that asks the dome to turn until a given part faces front (#322, 2026-09-08).
- "speed" names three unrelated quantities across one droid, and one of them is not a rate at all. The **Foot Drive**'s is a preset tier (`speedPreset`, `speedLimitMax`); the dome's is a signed percentage of full output (`speedPct`, -100..100); and a **Servo Output**'s is a **time** - how long a full throw takes, in milliseconds - because a builder checks a door with a stopwatch, not a rate table. Resolved by qualifying every use and never a bare "speed" in operator copy: **drive speed preset**, **dome speed**, and a Servo Output's **Motion Profile**, whose fields are named for what they are - *time to full throw* and *time to get up to speed* (#286, 2026-09-08).
- "named sound" and "**Named Track**" are one concept under two names across three surfaces: `data/sound.html:106` heads the card **Named Sounds**, `docs/api.md:624` and the `/api/audio/tracks` route say **named tracks**, and `docs/sequence-authoring.md:167` manages both in a single sentence. Resolved by **Named Track** everywhere, matching this ledger and the API. It is the same finding #288 closed for `Learned Sequences` against `Sequences`, one subsystem along - and worth reading as evidence that the sweep #298 owns should cover page headings, not only nav labels (#326, 2026-09-08).
