...
void test_static_tcp_diagnostics_format_as_json_object() {
    const WebResponseTcpDiagnostics diagnostics = {7U, 2U, 1U};
    char out[128];
    ...
    TEST_ASSERT_EQUAL_STRING(
        "{\"zeroProgress\":7,\"recoveries\":2,\"exhaustions\":1}",
        out);
}

// Issue #60: maximum boot-lifetime counters must fit the fixed local object
// budget used by buildStatusJson().
void test_static_tcp_diagnostics_max_values_fit_128_byte_budget() {
    const WebResponseTcpDiagnostics diagnostics = {UINT32_MAX, UINT32_MAX, UINT32_MAX};
    char out[128];
...
int main(int argc, char** argv) {
    ...
    RUN_TEST(test_static_tcp_diagnostics_max_values_fit_128_byte_budget);
    return UNITY_END();
}
