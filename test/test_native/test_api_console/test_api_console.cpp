...
// Registry has exactly 20 status-type entries (docs/action-registry.yaml) -
...
// sound.api.get-catalog/-get-mood-map across for the same reason again.
void test_operations_type_filter_lists_only_that_type() {
    ...
    TEST_ASSERT_EQUAL_UINT(1, countRecordsOfType(backend.sentBody, "begin"));
    TEST_ASSERT_EQUAL_UINT(1, countRecordsOfType(backend.sentBody, "end"));
    TEST_ASSERT_EQUAL_UINT_MESSAGE(20, countRecordsOfType(backend.sentBody, "item"),
