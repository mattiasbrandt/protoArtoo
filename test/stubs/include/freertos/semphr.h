...
// what the project's one remaining static mutex is: src/web/api_config.cpp's
// config write lock (xSemaphoreCreateMutexStatic), which every adapter takes
// and must be correct against. test_config_write_lock.cpp,
// test_console_module.cpp and test_console_concurrency.cpp drive it through
...
// (ADR 0011, amended): serialization moved down to the Commit Step, where one
// lock covers REST and Console alike instead of one adapter locking itself.
//
...
// src/web/api_config.cpp's config write lock.
