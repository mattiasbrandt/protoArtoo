...
test("Setup explains that auto-saved component changes require restart", () => {
  const html = readFileSync("data/setup.html", "utf8");
  assert.match(html, /Component changes save immediately\. Restart the controller to apply them\./);

  const env = loadPageModule("setup.js", { respond: () => ({}) });
  assert.equal(
    env.element("setup-save-summary").textContent,
    "💾 Auto-save · restart to apply",
    "the persistent auto-save summary must retain the restart boundary"
  );
});
...
test("component auto-save reports that controller restart is required", async () => {
  ...
  assert.match(env.element("feature-feedback").textContent, /Restart the controller to apply component changes\./);
