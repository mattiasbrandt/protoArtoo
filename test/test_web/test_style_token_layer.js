...
test("neither reserved colour is spent on a choice or on an answer that has not arrived", () => {
  ...
  // that was deleted - the four per-Mood hues, say - is caught too. A list of
  // exact selectors would pass vacuously the moment one of them stopped
  // existing, which is exactly what happened to the Moods.
  const surfaces = [
    [/^\.indicator$/, "a dashboard health lamp before the first status frame"],
    [/(^|,\s*)\.mood-btn\b/, "a Mood the operator chose"],
    [/(^|,\s*)\.opmode-btn\b/, "a Commanded Mode the operator chose"],
    [/#wifi-posture-card\[data-posture="standalone-ap"\]/, "a valid operator-selected posture"],
  ];
  const amber = TOKENS.get("--warning");
  const red = TOKENS.get("--danger");
  const spends = [];
  let scanned = 0;
  for (const rule of NON_ROOT) {
    const hit = surfaces.find(([pattern]) => pattern.test(rule.selector));
    if (!hit) continue;
    scanned += 1;
    for (const { property, value } of rule.declarations) {
      const resolved = resolve(value);
      if (resolved.includes(amber) || resolved.includes(red)) {
        spends.push(`${rule.selector} (${hit[1]}) { ${property}: ${value} }`);
      }
    }
  }
