...
def main() -> int:
    ...
    if not staged:
        # Block any commit when tracked generated version web assets are dirty
        # but not staged. extract_version.py updates them on every build; when
        # that build belongs to the slice being committed, the refreshed files
        # are part of the same logical change.
        dirty_issues = _version_file_issues(repo_dir)
        if dirty_issues:
            _deny(
                "Commit blocked: data/fw-version.json and data/fs-version.json are tracked "
                "generated web assets refreshed by 'pio run'. They carry the firmware/filesystem "
                "version shown by the board UI and kept in Git for release traceability. "
                "When the build belongs to the active code slice, they must be committed with "
                "the source changes from that same slice. If the build was incidental and outside "
                "the active slice/upload/release workflow, discard these dangling version changes "
        ...
        if version_issues:
            problems.append(
                "fw-version.json / fs-version.json are tracked generated web assets that must be fully staged — "
                "they carry the firmware/filesystem version shown by the board UI and ship as LittleFS files. "
