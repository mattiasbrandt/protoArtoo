...
void formatAudioStatusJson(char* buf, size_t bufSize, const char* driverName, bool linkOk,
    ...
    const char* playSt = (playState == 0x00)   ? "stop"
                         ...
                         : (playState == 0x02) ? "paused"
                                               : "unknown";
    // device labels per datasheet: 0=USB 1=SD/TF 2=FLASH 0xFF=none/unknown
    const char* devStr = (device == 0x00)   ? "USB"
                         ...
                         : (device == 0xFF) ? "none"
                                            : "unknown";
    snprintf(buf, bufSize,
             "{\"driver\":\"%s\",\"link_ok\":%s,\"active\":%s,"
             "\"play_state\":\"%s\",\"device\":\"%s\","
             "\"total_tracks\":%u,\"current_track\":%u}",
             driverName, linkOk ? "true" : "false", active ? "true" : "false", playSt, devStr,
             (unsigned)totalTracks, (unsigned)currentTrack);
}
